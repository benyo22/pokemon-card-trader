<?php
session_start();
include_once("storage/userstorage.php");
include_once("storage/cardstorage.php");    
include_once("auth.php");

//data
$card_storage = new CardStorage();
$cards = $card_storage->findAll();

$user_storage = new UserStorage();
$auth = new Auth($user_storage);

//check logged in user
if($auth->is_authenticated()){
    $user = $user_storage->findById($auth->authenticated_user()['id']);
}
else{
    die("Not logged in! Please go back! <a href=\"home.php\">Home</a>");
}

if(!$auth->isAdmin($auth)){
    $emptycards = $user['limit'] - count($user['cards']);
}
?>

<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>IKémon | User details</title>
    <link rel="stylesheet" href="styles/main.css">
    <link rel="stylesheet" href="styles/cards.css">
    <link rel="stylesheet" href="styles/details.css">
    <link rel="icon" type="image/x-icon" href="data:image/png;base64,">
</head>

<body>
    <header>
        <h1>
            IKémon | 
            <a href="home.php">Home</a> | 
            <?php if($auth->isAdmin($auth)) : ?>
                <a href="newcard.php">New Card</a> |
            <?php endif; ?>
            > My Cards < |
            <a href="logout.php">Log Out</a>  |
        </h1>
    </header>
    
    <?php if($auth->is_authenticated()) : ?>
        <div class="header-user">
            <span>Username: <?= $user['username'] ?>
            <br>
            <?php if(!$auth->isAdmin($auth)) echo "Money: {$user['money']} <span class=\"icon\">💰</span>"; ?></span>
        </div>
    <?php endif; ?>
    
    <main id="user-details-main">
        <div id="card-list-container">
            <div id="user-details">
                <div class="row">
                    <div class="first-col">
                        <p class="details-username">Username: <?= $user['username'] ?></p>
                        <p class="details-email">E-mail: <?= $user['email'] ?></p>
                    </div>
                    <div class="second-col">
                        <p class="details-money">Money: <?= $user['money'] ?><span class="icon">💰</span></p>
                        <p class="details-roles">Role(s): <?php foreach($user['roles'] as $role){echo $role;} ?></p>
                    </div>
                </div>
                <?php if(!$auth->isAdmin($auth)) : ?>
                    <br>
                    <p class="details-slots">Slots remaining: <?php echo $user['limit'] - count($user['cards']) ?></p>
                    <br>
                    <a href="newslot.php" id="buyslot">Buy slot for <?= $user['slotprice'] ?><span class="icon">💰</span></a>
                    <a href="sellslot.php" id="sellslot">Sell slot for 100<span class="icon">💰</span></a>
                <?php endif; ?>
            </div> 

            <div id="card-list-details">
                <?php foreach($user['cards'] as $card) : ?>
                    <div class="pokemon-card-details">
                        <div class="image clr-<?= $cards[$card]['type'] ?>">
                            <img src="<?= $cards[$card]["image"] ?>" alt="">
                        </div>
                        <div class="details">
                            <h2><a href="details.php?id=card<?= $cards[$card]["id"] ?>"><?= $cards[$card]["name"] ?></a></h2>
                            <span class="card-type"><span class="icon">🏷</span> <?= $cards[$card]["type"] ?></span>
                            <span class="attributes">
                                <span class="card-hp"><span class="icon">❤</span> <?= $cards[$card]["hp"] ?></span>
                                <span class="card-attack"><span class="icon">⚔</span> <?= $cards[$card]["attack"] ?></span>
                                <span class="card-defense"><span class="icon">🛡</span> <?= $cards[$card]["defense"] ?></span>
                            </span>
                        </div>

                        <?php if(!$auth->isAdmin($auth)) : ?>
                            <a href="sell.php?id=<?= "card{$cards[$card]["id"]}" ?>">
                                <div class="buy">
                                    <span class="card-price">Sell for <?= (int)($cards[$card]['price'] * 0.9) ?></span>
                                </div>
                            </a>
                        <?php else : ?>
                            <a href="editcard.php?id=card<?= $cards[$card]["id"] ?>">
                            <div class="buy">
                                    <span class="card-price">Edit</span>
                                </div>
                            </a>
                        <?php endif; ?>
                    </div>
                    </a>
                <?php endforeach; ?>
            </div>
        </div>
    </main>

    <footer>
        <p>IKémon | ELTE IK Webprogramozás</p>
    </footer>
</body>

</html>