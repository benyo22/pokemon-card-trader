<?php
session_start();
include_once("storage/userstorage.php");
include_once("storage/cardstorage.php");    
include_once("auth.php");

//functions
function canBuyCard($card, $auth, $user){
    if($auth->is_authenticated() && !$auth->isAdmin($auth) && !$card['hasowner'] && count($user['cards']) < $user['limit'])
        return true;
    return false;
}

//data
$card_storage = new CardStorage();
$cards = $card_storage->findAll();

$user_storage = new UserStorage();
$auth = new Auth($user_storage);

$user = null;

//check logged in user
if($auth->is_authenticated()){
    $user = $user_storage->findById($auth->authenticated_user()['id']);
}

//check filter
$isfilter = false;
$filterType = "";
if(isset($_POST['pokemonType']) && $_POST['pokemonType'] !== ''){
    $isfilter = true;
    $filterType = $_POST['pokemonType'];
}

?>

<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>IKémon | Home</title>
    <link rel="stylesheet" href="styles/main.css">
    <link rel="stylesheet" href="styles/cards.css">
    <link rel="icon" type="image/x-icon" href="data:image/png;base64,">
</head>

<body>
    <?php if($auth->is_authenticated()) : ?>
        <div class="header-user">
            <span>Username: <?= $user['username'] ?>
            <br>
            <?php if(!$auth->isAdmin($auth)) echo "Money: {$user['money']} <span class=\"icon\">💰</span>"; ?></span>
        </div>
    <?php endif; ?>
    <header>
        <h1>
            IKémon | 
            > Home < | 
            <?php if(!$auth->is_authenticated()) : ?>
                <a href="login.php">Log In</a> | 
            <?php endif; ?>
            <?php if(!$auth->is_authenticated()) : ?>
                <a href="register.php">Register</a> | 
            <?php endif; ?>
            <?php if($auth->isAdmin($auth)) : ?>
                <a href="newcard.php">New Card</a> |
            <?php endif; ?>
            <?php if($auth->is_authenticated()) : ?>
                <a href="userdetails.php">My Cards</a>  |
            <?php endif; ?>
            <?php if($auth->is_authenticated()) : ?>
                <a href="logout.php">Log Out</a>  |
            <?php endif; ?>
            
        </h1>
    </header>


    <div class="container-address">
        <h2>
            Begin your quest, forge your legend!
        </h2>
        <p>Welcome to IKémon! Unleash an epic NFT adventure, capturing unique digital companions and rare treasures. This isn't just a marketplace; it's your journey to become a legend. Let the adventure begin!</p>
    </div>

    <form method="post" action="" class='filter-form'>
        <label for="pokemonType" class="filter-label">Select a Pokémon type:</label>
        <select name="pokemonType" id="pokemonType">
            <option value="">All Types</option>
            <?php
                $allowedTypes = [
                    "Normal", "Fire", "Water", "Grass", "Flying", "Fighting", "Poison", "Electric",
                    "Ground", "Rock", "Psychic", "Ice", "Bug", "Ghost", "Steel", "Dragon", "Dark", "Fairy"
                ];

                // Loop through the types to create options
                foreach ($allowedTypes as $type) {
                    echo "<option value='" . strtolower($type) . "'>$type</option>";
                }
            ?>
        </select>
        <input type="submit" value="Filter" class="filter-button">
    </form>

    <div id="content">
        <div id="card-list">
            <?php foreach($cards as $card) : ?>
                <?php if(!$isfilter) : ?>
                    <div class="pokemon-card" id="<?= $card['id'] ?>">
                        <div class="image clr-<?= $card['type'] ?>">
                            <img src="<?= $card["image"] ?>" alt="">
                        </div>
                        <div class="details">
                            <h2><a href="details.php?id=card<?= $card["id"] ?>"><?= $card["name"] ?></a></h2>
                            <span class="card-type"><span class="icon">🏷</span> <?= $card["type"] ?></span>
                            <span class="attributes">
                                <span class="card-hp"><span class="icon">❤</span> <?= $card["hp"] ?></span>
                                <span class="card-attack"><span class="icon">⚔</span> <?= $card["attack"] ?></span>
                                <span class="card-defense"><span class="icon">🛡</span> <?= $card["defense"] ?></span>
                            </span>
                        </div>
                        <?php if(canBuyCard($card, $auth, $user)) : ?>
                            <a href="buycard.php?id=card<?= $card['id'] ?>">
                                <div class="buy">
                                    <span class="card-price"><span class="icon">💰</span> <?= $card["price"] ?></span>
                                </div>
                            </a>
                        <?php elseif($auth->isAdmin($auth) && !$card['hasowner']) : ?>
                            <a href="editcard.php?id=card<?= $card['id'] ?>">
                                <div class="buy">
                                    <span class="card-price">Edit</span>
                                </div>
                            </a>
                        <?php elseif($auth->isAdmin($auth) && $card['hasowner']) : ?>
                            <div class="cantbuy">
                                <span class="card-price">Edit</span>
                            </div>
                        <?php else: ?>
                            <div class="cantbuy">
                                <span class="card-price"><span class="icon">💰</span> <?= $card["price"] ?></span>
                            </div>
                        <?php endif; ?>
                    </div>
                    </a>
                <?php elseif($isfilter && $card['type'] === $filterType) : ?>
                    <div class="pokemon-card">
                        <div class="image clr-<?= $card['type'] ?>">
                            <img src="<?= $card["image"] ?>" alt="">
                        </div>
                        <div class="details">
                            <h2><a href="details.php?id=card<?= $card["id"] ?>"><?= $card["name"] ?></a></h2>
                            <span class="card-type"><span class="icon">🏷</span> <?= $card["type"] ?></span>
                            <span class="attributes">
                                <span class="card-hp"><span class="icon">❤</span> <?= $card["hp"] ?></span>
                                <span class="card-attack"><span class="icon">⚔</span> <?= $card["attack"] ?></span>
                                <span class="card-defense"><span class="icon">🛡</span> <?= $card["defense"] ?></span>
                            </span>
                        </div>
                        <?php if(canBuyCard($card, $auth, $user)) : ?>
                            <a href="buycard.php?id=card<?= $card['id'] ?>">
                                <div class="buy">
                                    <span class="card-price"><span class="icon">💰</span> <?= $card["price"] ?></span>
                                </div>
                            </a>
                        <?php elseif($auth->isAdmin($auth) && !$card['hasowner']) : ?>
                            <a href="editcard.php?id=card<?= $card['id'] ?>">
                                <div class="buy">
                                    <span class="card-price">Edit</span>
                                </div>
                            </a>
                        <?php else: ?>
                            <div class="cantbuy">
                                <span class="card-price"><span class="icon">💰</span> <?= $card["price"] ?></span>
                            </div>
                        <?php endif; ?>
                    </div>
                    </a>
                <?php endif;?>
            <?php endforeach; ?>     
        </div>
    </div>

    <footer>
        <p>IKémon | ELTE IK Webprogramozás</p>
    </footer>
</body>

</html>